...
const task = (id: string, title: string): Task => ({ id, title, agent_type: "react", updated_at: "", pinned: false, starred: false, archived: false });
...
describe("filterTasksByTitle", () => {
  ...
  it("null 标题不抛错且不匹配", () => {
    const mixed: Task[] = [
      task("1", "写周报"),
      { id: "2", title: null as unknown as string, agent_type: "react", updated_at: "", pinned: false, starred: false, archived: false },
    ];
