...
export class PostgresCheckpointStore implements CheckpointStore {
  ...
  /** 包一层事务 + set_config('app.tenant_id', ...)，跟 loadForTenant 等方法用同样的机制——
   * tenantId 直接从 checkpoint.tenantId 取，不需要新增一个 saveForTenant 方法（这是 spec 里
   * 明确的设计决定：save() 本身已经知道要写入哪个租户，不存在"调用方可能传错租户"的歧义）。
   * 这一步是 RLS 真正生效（Task 5/9）之后 checkpoint 创建/更新不被 WITH CHECK 拒绝的必要前提。 */
  async save(checkpoint: Checkpoint): Promise<void> {
    ...
    await this.db.transaction(async (tx) => {
      await tx.execute(sql`select set_config('app.tenant_id', ${checkpoint.tenantId}, true)`);
      await tx
        .insert(checkpoints)
        .values(values)
        .onConflictDoUpdate({
